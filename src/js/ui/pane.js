  ...
  var CrmPowerPane = {
    ...
    UI: {
        ...
        (this.Parameters = []),
          ...
          (this.Initialize = function () {
            ...
            $popup
              .find("h1")
              .html(this.Header)
              .toggle(this.Header != null);
            $popup
              .find("p")
              .html(this.Description)
              .toggle(this.Description != null);
            $popup.find("li").remove();

            $popup.unbind().keyup(function (event) {
              if (event.key === "Enter")
                $("#crm-power-pane-popup-ok") &&
                  $("#crm-power-pane-popup-ok").click();
              else if (event.key === "Escape")
                $("#crm-power-pane-popup-cancel") &&
                  $("#crm-power-pane-popup-cancel").click();
            });
            return $popup;
          }),
      ...
      SetButtonBackgrounds: function () {
        var color = "#001ca5";
        if (
          CrmPowerPane.TargetFrame.GetApplicationType() ==
          CrmPowerPane.ApplicationType.DynamicsCRM
        ) {
          color = $("#crmMasthead").css("background-color");
        } else if (
          CrmPowerPane.TargetFrame.GetApplicationType() ==
          CrmPowerPane.ApplicationType.Dynamics365
        ) {
          color = $("div[data-id=topBar]").css("background-color");
        }

        color =
          color == "rgb(0, 32, 80)" || color == "rgb(0, 20, 51)"
            ? "#001ca5"
            : color;
    ...
    ServiceOperations: {
      ExecuteQuery: function (query, isAsync, successCallback, errorCallback) {
        ...
        req.open(
          "GET",
          Xrm.Page.context.getClientUrl() + "/api/data/v8.2/" + query,
          isAsync
        );
        ...
        req.onreadystatechange = function () {
          if (this.readyState == 4) {
            ...
            if (this.status == 200) {
              ...
              if (errorCallback) errorCallback(JSON.parse(this.response).error);
            }
          }
        };
        req.send();
      ...
      RetrieveEntityMetadata: function (entityMetadataId) {
        CrmPowerPane.ServiceOperations.ExecuteQuery(
          "EntityDefinitions(" +
          entityMetadataId +
          ")?$select=LogicalName&$expand=Attributes($select=LogicalName,DisplayName)",
          true,
          function (data) {
            console.log(data.value);
          },
      ...
      GetContent: function () {
        try {
          ...
          if (applicationType == CrmPowerPane.ApplicationType.DynamicsCRM) {
            var frameLevel = $("iframe").filter(function () {
              return $(this).css("visibility") == "visible";
            })[1];
            if (!frameLevel) {
              frameLevel = $("iframe").filter(function () {
                return $(this).css("visibility") == "visible";
              })[0];
            }
            return frameLevel.contentWindow;
          } else if (
            applicationType == CrmPowerPane.ApplicationType.Dynamics365
          ) {
            return window;
          } else {
            return null;
    ...
    Errors: {
      ...
      WrongPageWarning: function () {
        CrmPowerPane.UI.ShowNotification(
          "You need to open a record form to use this action. ",
          "warning"
        );
      },
    },
    RegisterEvents: function () {
      ...
      var _getSelectElement = function (attributeLogicalName) {
        var $select = Content.$(
          "select.ms-crm-SelectBox[attrname=" + attributeLogicalName + "]"
        );
      ...
      $(document).on("click", "#crm-power-pane-button", function (e) {
        $(".crm-power-pane-sections").slideToggle(
          CrmPowerPane.Constants.SlideTime
        );
        e.stopPropagation();
      });
      ...
      $(document).on("click", function () {
        $(".crm-power-pane-sections")
          .delay(100)
          .slideUp(CrmPowerPane.Constants.SlideTime);
      });
      ...
      $("#record-properties").click(function () {
        try {
          ...
          if (
            Content.Mscrm &&
            Content.Mscrm.RibbonActions &&
            Content.Mscrm.RibbonActions.openFormProperties
          ) {
      ...
      $("#go-to-record").click(function () {
        try {
          CrmPowerPane.UI.BuildInputPopup(
            ...
            function (popupObj) {
              ...
              if (params.entityname.value && params.recordid.value) {
                var linkProps = [
                  Xrm.Page.context.getClientUrl() + "/main.aspx",
                ];
          ...
          CrmPowerPane.UI.ShowNotification(
            "An error ocurred while redirecting to specified record.",
            "error"
          );
        }
      });
      ...
      $("#user-info").click(function () {
        try {
          function getUserRoles() {
            ...
            var results =
              requestResults.results[0].systemuserroles_association.results;
          ...
          function getUserTeams() {
            ...
            var requestResults = eval("(" + service.responseText + ")").d;
            var results =
              requestResults.results[0].teammembership_association.results;
      ...
      $("#show-all-fields").click(function () {
        try {
          ...
          CrmPowerPane.UI.ShowNotification(
            "Visibility of all fields updated to visible."
          );
        } catch (e) {
          CrmPowerPane.Errors.WrongPageWarning();
        }
      ...
      $("#disable-field-requirement").click(function () {
        try {
          ...
          CrmPowerPane.UI.ShowNotification(
            "Required level of all fields updated to none."
          );
        } catch (e) {
          CrmPowerPane.Errors.WrongPageWarning();
        }
      ...
      $("#schema-names").click(function () {
        try {
          ...
          if (updateStatus == "update")
            CrmPowerPane.UI.ShowNotification(
              "All labels updated to schema name."
            );
          else if (updateStatus == "rollback")
            CrmPowerPane.UI.ShowNotification(
              "Schema name updates rolled back."
            );
      ...
      $("#schema-names-as-desc").click(function () {
        try {
                  ...
                  $label.off("click").click(function () {
                    ...
                    if (canCopy) {
                      ...
                      (tempTextArea.style.position = "absolute"),
                        (tempTextArea.style.top = -9999),
                        (tempTextArea.style.left = -9999),
                        ...
                        (tempTextArea.value = attributeLogicalName),
                      ...
                      try {
                        ...
                        if (didCopy) {
                          ...
                          CrmPowerPane.UI.ShowNotification(
                            "Copying failed. Please copy it yourself.",
                            "error"
                          );
                        }
                      } catch (i) {
          ...
          CrmPowerPane.UI.ShowNotification(
            "Schema name mode is activated for descriptions. You can copy it with label click."
          ); // ui message will change
        } catch (e) {
          CrmPowerPane.Errors.WrongPageWarning();
        }
      });
      ...
      $("#schema-names-in-brackets").click(function () {
        try {
          ...
          if (updateStatus == "update")
            ...
            CrmPowerPane.UI.ShowNotification(
              "Removed schema names in brackets."
            );
        ...
        Xrm.Page.ui.controls.forEach(function (control) {
          try {
            if (control.getControlType() === "lookup") {
              ...
              var openInNewWindowLink = $(
                '<a id="' +
                linkId +
                '" class="crm-power-pane-lookup-link" alt="Open this record in a new window" title="Open this record in a new window"  style="cursor: pointer;margin-left: 5px">' +
                externalIcon +
                "</a>"
              );
        ...
        try {
          Xrm.Page.ui.controls.forEach(function (control) {
            if (
              ...
              control.getControlType() == "optionset"
            ) {
              ...
              if ($options && $options.length > 0) {
                ...
                for (var i = 0; i < $options.length; i++) {
                  ...
                  if (changedOrReverted == "changed") {
                    CrmPowerPane.UI.ShowNotification(
                      "Added value property to the option labels for all optionset. Like that <b>#value#</b>"
                    );
                  } else if (changedOrReverted == "reverted") {
                    CrmPowerPane.UI.ShowNotification(
                      "Removed value property from the option labels for all optionset.",
                      "error"
                    );
                  }
                }
      ...
      $("#crm-diagnostics").click(function () {
        window.open(
          Content.Xrm.Page.context.getClientUrl() +
      ...
      $("#mobile-client").click(function () {
        ...
        window.open(
          url +
          "/nga/main.htm?org=" +
          Content.Xrm.Page.context.getOrgUniqueName() +
          "&server=" +
          url
        );
      });
      ...
      $("#clear-all-notifications").click(function () {
        try {
          ...
          CrmPowerPane.UI.ShowNotification(
            "Notifications of all fields have been cleared."
          );
        } catch (e) {
          CrmPowerPane.Errors.WrongPageWarning();
        }
      ...
      $("#open-entity-editor").click(function () {
        try {
          ...
          CrmPowerPane.UI.ShowNotification(
            "An error ocurred while redirecting to entity editor.",
            "error"
          );
        }
      });
              ...
              function (popupObj) {
                ...
                if (outputParams) {
                  popupObj.Description =
                    ...
                    control.getLabel() +
                    "(" +
                    popupObj.Parameters.fieldname.value +
                    ")</b> is " +
                    controlType +
                  ...
                  CrmPowerPane.UI.ShowNotification(
                    "The control type of your field is not recognized.",
                    "warning"
                  );
                }
              },
                ...
                if (fieldName) {
                  ...
                  if (control && control.setFocus) {
                    ...
                    if (control.getVisible() == false) {
                      hiddenMessage =
                        "Control is <b>hidden</b>, it changed as visible.";
                      control.setVisible(true);
                    }
                    CrmPowerPane.UI.ShowNotification(
                      "Focused on the " + fieldName + " field. " + hiddenMessage
                    );
                    Content.$("#" + control.getName()).css(
                      "background",
                      "#FFFF00"
                    );
                  }
                  ...
                  CrmPowerPane.UI.ShowNotification(
                    fieldName + " field could not be found.",
                    "warning"
                  );
                }
              }
      ...
      $("#crm-power-pane").on("click", ".crm-power-pane-copy", function () {
        $(".crm-power-pane-copy")
          .removeClass("crm-power-pane-copied")
          .html("Copy it!");
      ...
      $("#crm-power-pane-fetchxml-popup-container ul li").click(function () {
        ...
        $("#crm-power-pane-fetchxml-popup-container ul li").removeClass(
          activeClass
        );
      ...
      $("#solutions").click(function () {
        window.open(
          Xrm.Page.context.getClientUrl() +
          "/tools/Solution/home_solution.aspx?etc=7100",
          "_blank"
        );
      });
      ...
      $("#go-to-create-form").click(function () {
        try {
          CrmPowerPane.UI.BuildInputPopup(
            ...
            function (popupObj) {
              ...
              if (params.entityname.value) {
                ...
                CrmPowerPane.UI.ShowNotification(
                  "Entity name is required. Please fill it and try again.",
                  "warning"
                );
              }
            }
          ...
          CrmPowerPane.UI.ShowNotification(
            "An error ocurred while redirecting to specified create form.",
            "error"
          );
        }
      });
      ...
      $("#open-form-editor").click(function () {
        try {
          ...
          params.push(
            "&etn=" + Xrm.Page.data.entity.getEntityName().toLowerCase()
          );
          ...
          CrmPowerPane.UI.ShowNotification(
            "An error ocurred while redirecting to form editor.",
            "error"
          );
        }
      });
      ...
      $("#open-webapi").click(function () {
        try {
          ...
          var shortVersion = apiVersion.substring(
            3,
            apiVersion.indexOf(".") - 1
          );
          ...
          CrmPowerPane.UI.ShowNotification(
            "An error occured opening the Web API URL for this record."
          );
        }
      });
    },
